...
{ static double meanx = 0; 
  ...
  public static boolean isPrefixedSequence(Vector[] xs, Vector[] ys, String sent, String tent, ModelSpecification mod) 
   ...
   public static boolean isSubsetSet(Vector[] xs, Vector[] ys, String sent, String tent, ModelSpecification mod) 
   { for (int i = 0; i < xs.length && i < ys.length; i++) 
     { Vector xval = xs[i]; 
       Vector yval = ys[i];
       int xsize = xval.size(); 
       int ysize = yval.size(); 
       if (xsize < ysize) 
       { Vector ytail = mod.correspondingObjectSubset(sent,tent,xval,yval); 
         if (ytail != null) 
   ...
   public static Vector commonSubsetSet(Vector[] xs, Vector[] ys, String sent, String tent, ModelSpecification mod) 
   { java.util.Set unionsets = new java.util.HashSet(); 
	 ...
	 if (unionsets.size() == 1)
	 { Vector sourceadded = mod.getSourceObjects(sent,tent,added); 
