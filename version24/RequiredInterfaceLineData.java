/*
      * Classname : RequiredInterfaceLineData
      * 
...
public class RequiredInterfaceLineData extends LineData
{  
  public RequiredInterfaceLineData(int xs, int ys, int xe, 
  ...
  { String cnt = label.substring(1,label.length()-1); 
    ...
    RequiredInterfaceLineData ld = 
      new RequiredInterfaceLineData(xstart, ystart, xend, 
                   yend, count, linetype); 
  ...
  { g.setColor(Color.black);
    ...
    // g.drawArc(xx, yy, 10, 10, 270, 0);

    if (Math.abs(yend - ystart) <= 50)
    { if (xend > xstart)
      { g.drawArc(xx, yy-5, 10, 10, 90, 180); }
      else 
      { g.drawArc(xx-5, yy, 10, 10, 270, 180); }
    }  
    else if (yend > ystart)
    { if (xend < xstart) 
      { g.drawArc(xx + 5, yy + 5, 10, 10, 0, 180); } 
      else 
      { g.drawArc(xx, yy, 10, 10, 0, 180); }
    }  
    else 
    { if (xend < xstart) 
      { g.drawArc(xx + 5, yy - 5, 10, 10, 180, 180); } 
      else 
      { g.drawArc(xx - 5, yy - 5, 10, 10, 180, 180); } 
    } 
