...
{ Attribute src; 
  ...
  { Vector res = new Vector(); 
    ...
    { // assume auxVariables.size() == 2
      ...
      if (srcvalue instanceof BinaryExpression && auxVariables.size() >= 2)
      { BinaryExpression sbe = (BinaryExpression) srcvalue; 
        ...
        { Expression lbe = sbe.getLeft(); 
          ...
          if ((rbe + "").equals(var1.getName()) && 
              rbe instanceof BinaryExpression)  // var1 + (sep + var2)
          { delim = (BasicExpression) ((BinaryExpression) rbe).getLeft();
            System.out.println(">> Delimiter = " + delim); 
                   ...
                   (((BinaryExpression) lbe).getLeft() + "").equals(var1.getName()))
          { // (var1 + sep) + var2
            delim = (BasicExpression) ((BinaryExpression) lbe).getRight();
            System.out.println(">> Delimiter = " + delim); 
          } 
        } 
      } 
...
{ if (path.size() == 0)
  ...
  { // For each feature mapping f |--> g, checks for each 
    ...
    { ObjectSpecification sobj = (ObjectSpecification) srcobjs.get(i); 
      ...
      if (src.isNumeric() && trg.isNumeric())
      { double srcval = sobj.getNumeric(src.getName()); 
	    xs[i] = srcval;
        Double xv = new Double(srcval);  
        xlist.add(xv); 
        double trgval = tobj.getNumeric(trg.getName());
	    ys[i] = trgval; 
        ymap.put(xv,new Double(trgval)); 
		 
        if (srcval == trgval) 
        { System.out.println(">> objects " + sobj + " " + tobj + " satisfy matching " + this); } 
        else 
	  ...
	  { String srcstr = sobj.getString(src.getName()); 
	    String trgstr = tobj.getString(trg.getName());
		xstrs[i] = srcstr; 
		ystrs[i] = trgstr; 
	  ...
	  { Vector srcvect = sobj.getCollection(src.getName()); 
	    Vector trgvect = tobj.getCollection(trg.getName()); 
	...
	if (!valid && src.isNumeric() && trg.isNumeric())
	{ // check if the x and y are constant, or 
     ...
     { if (AuxMath.isConstantSet(yvect))
       ...
       boolean subsetted = AuxMath.isSubsetSet(xvect, yvect, sent, tent, mod); 
	 if (subsetted)
	 { System.out.println(">>> Target is " + src + " + some elements ");  
